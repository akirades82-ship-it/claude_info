...
**트레일링 스톱 vs 고정 익절 비교 (2026-07-14, 중요):** 사용자가 실제 원하는 방식은 "오르는 동안 계속 보유 → 최고점 대비 %하락시 매도(트레일링) → 급락시 손절가"였음(고정 익절과 다름, 확인 후 재검증함). 갭확인후진입+64건 표본 기준: **트레일 폭 1~3%는 분단위 노이즈에 계속 털려서 손해(-0.3~0.5%), 6%+로 넓혀야 플러스로 돌아서는데 그건 사실상 "그냥 마감까지 보유"(대조군 +0.97%)와 결과가 거의 같아지기 때문 — 트레일링 메커니즘 자체의 부가가치는 이 데이터에서 뚜렷하지 않음.** 지금까지 나온 후보 중 가장 단순하고 결과도 괜찮은 건 여전히 "고정 익절(목표 도달시 즉시 매도)+손절 4~7%" (+0.51~0.76%). 사용자의 트레일링 직관은 합리적이지만 실측상 최선의 실행 형태는 아니었음 — 이 결론이 표본(대부분 하락장 시기, 코스닥 한정)에 국한된 것인지는 미확인.
