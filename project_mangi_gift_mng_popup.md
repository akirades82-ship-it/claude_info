...
**2026-07-06 bug fix (user-reported runtime error):** `java.sql.SQLSyntaxErrorException: ORA-00904: "MNG"."UPD_DM": 부적합한 식별자` — `TB_MEMBER_MANGI_GIFT_MNG` has no `UPD_DM` column (it does have `REG_MAN`, `REG_DM`, `UPD_MAN` — confirmed by the error surfacing only on `UPD_DM`, the last-referenced audit column in the SELECT list, meaning the earlier ones resolved fine). Removed all 3 `UPD_DM` references in `DlwMangiMngMap.xml` (`getMangiGiftMngList` SELECT, `updateMangiGiftMng` SET, `deleteMangiGiftMng` SET) while keeping `UPD_MAN`. Also separately found and fixed: `insertMangiGiftMng` never populated the `SEQ` PK column at all (neither in the mapper XML nor the Java service) — added `(SELECT NVL(MAX(SEQ),0)+1 FROM ...)` matching the sibling `insertMemberMangiGiftset` pattern in `DlwPayMap.xml`. Could not verify directly against the DEV Oracle DB (no sqlplus/oracledb client available in this WSL session) — both fixes are inferred from the error message and existing code conventions, not confirmed against live schema.
